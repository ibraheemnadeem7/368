"""
wall_greedy_v2.py  –  Improved greedy wall designer
...
def _place_uniform(wall, ordering, gap, scoring_data):
    """
    Place artworks left-to-right with a uniform gap, starting from the
    left wall edge (x = 0).  Works that don't fit are skipped.
    """
    mn, mx = _gap_bounds(scoring_data)
...
def _place_anchor_right(wall, others_order, anchor, gap, scoring_data):
    ...
    anchor_x = max(0.0, min(anchor_x, wall_width - anchor_w))

    # Pack others to the left of anchor
    n = len(others_order)
    sum_w = sum(_f(a.get('width_ft')) for a in others_order)

    # start_x + sum_w + (n−1)*gap + gap = anchor_x
    start_x = anchor_x - sum_w - n * gap

    if start_x < 0.0:
        # Recompute gap so others fit starting at x=0
        if n > 0 and anchor_x > sum_w:
            gap = max(mn, min(mx, (anchor_x - sum_w) / n))
        start_x = max(0.0, anchor_x - sum_w - n * gap)
